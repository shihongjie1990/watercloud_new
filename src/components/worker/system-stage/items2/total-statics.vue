<template>
  <div id="stage_bar"
       style="width: 3rem;height: 2.3rem;"></div>
</template>

<script>
export default {
  data() {
    return {
      staticsData: {
        cpu: 1.34,
        totalCpu: 40,
        gib: 56.32,
        totalGib: 156.07,
        localStorage: 0.34,
        totallocalStorage: 4.33,
        groups: 186,
        totalGroups: 660
      }
    }
  },
  watch: {
    staticsData: {
      handler(curData, oldData) {
        let datas = this.handlerData(curData)
        this.initBar(datas)
      },
      deep: true
    }
  },
  methods: {
    handlerData(data) {
      let xAxis = [`CPU core\n(${data.cpu.toFixed(2)}/${data.totalCpu.toFixed(2)})`, `内存 GiB\n(${data.gib.toFixed(2)}/${data.totalGib.toFixed(2)})`, `本地存储 TB\n(${data.localStorage.toFixed(2)}/${data.totallocalStorage.toFixed(2)})`, `容器组\n(${data.groups}/${data.totalGroups})`]
      let datas = [parseFloat((data.cpu * 100 / data.totalCpu).toFixed(2)), parseFloat((data.gib * 100 / data.totalGib).toFixed(2)), parseFloat((data.localStorage * 100 / data.totallocalStorage).toFixed(2)), parseFloat((data.groups * 100 / data.totalGroups).toFixed(2))]
      return {
        xAxis: xAxis,
        data: datas
      }
    },
    initBar(data) {
      // 颜色
      let lightBlue = {
        type: 'linear',
        x: 0,
        y: 0,
        x2: 0,
        y2: 1,
        colorStops: [{
          offset: 0,
          // color: 'rgba(41, 121, 255, 1)'
          color: 'rgb(0, 92, 194)'
        }, {
          offset: 1,
          // color: 'rgba(0, 192, 255, 1)'
          color: 'rgb(50, 127, 167)'
        }],
        globalCoord: false
      }
      // 纹理
      let piePane = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAKElEQVQ4jWP8//8/AwXgPxMluhkYGBhGDRg1YNQAKhnAwsDAQFF+BgBtSwUd6uvSywAAAABJRU5ErkJggg=='
      let piePatternImg = new Image()
      piePatternImg.src = piePane
      // 图标
      let cpu = 'image://data:image/png;base64,'
      let memory = 'image://data:image/png;base64,'
      let disk = 'image://data:image/png;base64,'
      let group = 'image://data:image/png;base64,'

      // 指定图标的配置项和数据
      let option = {
        tooltip: {
          show: false
        },
        grid: {
          top: '30%',
          left: '5%',
          right: '5%',
          bottom: '15%'
        },
        xAxis: {
          data: data.xAxis,
          offset: 10,
          axisTick: {
            show: false
          },
          axisLine: {
            show: false
          },
          axisLabel: {
            color: '#fff',
            fontSize: 12,
            interval: 0
          }
        },
        yAxis: {
          min: 0,
          max: 100,
          splitLine: {
            show: false
          },
          axisTick: {
            show: false
          },
          axisLine: {
            show: false
          },
          show: false
        },
        series: [{
          type: 'bar',
          label: {
            show: true,
            position: 'top',
            padding: 10,
            color: '#2979ff',
            fontSize: 14,
            formatter: '{c}%'
          },
          itemStyle: {
            color: lightBlue
          },
          barWidth: '40%',
          data: data.data,
          z: 10
        }, {
          type: 'bar',
          barGap: '-100%',
          itemStyle: {
            color: {
              image: piePatternImg,
              repeat: 'repeat'
            },
            opacity: 0.05
          },
          barWidth: '40%',
          data: [100, 100, 100, 100],
          z: 5
        }, {
          type: 'bar',
          barGap: '-100%',
          itemStyle: {
            color: '#10367e',
            opacity: 0.3
          },
          barWidth: '40%',
          data: [100, 100, 100, 100],
          z: 5
        }, {
          type: 'bar',
          barGap: '-100%',
          label: {
            normal: {
              show: true,
              position: 'top',
              textStyle: {
                color: '#000'
              }
            }
          },
          itemStyle: {
            color: '#445d8d',
            opacity: 0.2
          },
          barWidth: '40%',
          data: [100, 100, 100, 100],
          z: 5
        }, {
          name: 'glyph',
          type: 'pictorialBar',
          barGap: '-100%',
          symbolPosition: 'end',
          symbolSize: 64,
          symbolOffset: [0, '-150%'],
          data: [{
            value: 100,
            symbol: cpu
          }, {
            value: 100,
            symbol: memory
          }, {
            value: 100,
            symbol: disk
          }, {
            value: 100,
            symbol: group
          }]
        }]
      }

      let myChart = this.$echarts.init(document.getElementById('stage_bar'))
      myChart.setOption(option)
    },
    getCluster() {
      this.$http.get('/api/monitor/cluster_level_metric_data').then(res => {
        let data = res.data.body.results
        let staticsData = this.handlerArr2Obj(data)
        let totalStatics = {
          cpu: staticsData['cluster_cpu_usage'],
          totalCpu: staticsData['cluster_cpu_total'],
          gib: staticsData['cluster_memory_usage_wo_cache'] / Math.pow(1024, 3),
          totalGib: staticsData['cluster_memory_total'] / Math.pow(1024, 3),
          localStorage: staticsData['cluster_disk_size_usage'] / Math.pow(1024, 4),
          totallocalStorage: staticsData['cluster_disk_size_capacity'] / Math.pow(1024, 4),
          groups: staticsData['cluster_pod_running_count'],
          totalGroups: staticsData['cluster_pod_quota']
        }
        this.staticsData = totalStatics
      })
    },
    handlerArr2Obj(data) {
      let obj = {}
      data.map(item => {
        obj[item['metric_name']] = parseFloat(item.data.result[0].value[1])
      })
      return obj
    }
  },
  mounted() {
    this.getCluster()
  }
}
</script>
