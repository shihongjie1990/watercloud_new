<template>
  <div>
    ...
    <el-row class="tool-bar">
      ...
      <el-button @click="$emit('closeLimit')">取消</el-button>
    </el-row>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getSelectedUsers() {
      this.$http
        .get(`/api/permission/list?roleId=${this.roleId}`)
        .then(res => {
          if (res.code === 1002) {
    ...
    onConfirm() {
      ...
      if (selectedNodes.length > 0) {
        ...
        this.$http
          .post('/api/permission/assign', { permissionIds: ids, roleId: this.roleId }, { loading: { operation: true } })
          .then(res => {
            if (res.code === 1002) {
